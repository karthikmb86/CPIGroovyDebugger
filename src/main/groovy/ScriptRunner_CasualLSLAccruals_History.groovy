...
class ScriptRunner_CasualLSLAccruals_History {

    static void main(String[] args) {
        // --- 0) Inputs (override with program args if you want) ---
        // arg0 = script path, arg1 = sample xml path
        String scriptPath            = (args?.length ?: 0) > 0 ? args[0] : "src/main/groovy/CompareHistoricUPL-LSLs_NewAccrRulesCopy.groovy"

        String TimeAccountDetsUserFile = (args?.length ?: 0) > 2 ? args[2] : "samples/TimeAccDets_User1.xml"
        String UserLeaveDetailsFile = (args?.length ?: 0) > 2 ? args[2] : "samples/UserLeaveDetails1.xml"
        String AccrualRatesFile      = (args?.length ?: 0) > 3 ? args[3] : "samples/AccrualRates1.xml"
        String EmpEmploymentHistoricFile     = (args?.length ?: 0) > 1 ? args[1] : "samples/EmpEmployment_Historic1.xml"
        ...
        String TimeAccountDetsUserXML = readOrFallback(TimeAccountDetsUserFile, DEFAULT_FALLBACK_XML)
        String UserLeaveDetailsXML = readOrFallback(UserLeaveDetailsFile, DEFAULT_FALLBACK_XML)
        String AccrualRatesXML = readOrFallback(AccrualRatesFile, DEFAULT_FALLBACK_XML)
        String EmpEmploymentHistoricXML = readOrFallback(EmpEmploymentHistoricFile, DEFAULT_FALLBACK_XML)

        // --- 3) Build CPI-like Message, set body + expected properties ---
        def msg = new Message()
        //msg.setBody(sampleXml)
        msg.setProperty("TimeAccDets_User", TimeAccountDetsUserXML)
        msg.setProperty("UserLeaveDetails", UserLeaveDetailsXML)
        msg.setProperty("AccrualRates", AccrualRatesXML)
        msg.setProperty("EmpEmployment_Historic", EmpEmploymentHistoricXML)

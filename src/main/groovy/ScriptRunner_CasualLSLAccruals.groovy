...
class ScriptRunner_CasualLSLAccruals {

    static void main(String[] args) {
        // --- 0) Inputs (override with program args if you want) ---
        // arg0 = script path, arg1 = sample xml path
        String scriptPath            = (args?.length ?: 0) > 0 ? args[0] : "src/main/groovy/LSLAccrualCalculation.groovy"
        String EmpEmploymentFile     = (args?.length ?: 0) > 1 ? args[1] : "samples/EmpEmployment_FlagAddedFullUAT.xml"
        String TimeAccountDetailsFile = (args?.length ?: 0) > 2 ? args[2] : "samples/TimeAccountDetails_Enhanced.xml"
        ...
        msg.setProperty("TimeAccountDetails", TimeAccountDetailsXML)

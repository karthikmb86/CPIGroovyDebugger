...
//                        newTotalTeachingHours = priorServiceRecord.totalTeachingHours + (newServiceHours-targetHours)
        ...
        def newNode = createNewIncrementProgressionDetails(employeeData, employmentRecord, workScheduleDayModelAssignments, priorServiceRecord, nesaStatus,empJobHistory, inactiveTimesheets)
...
//Added inactiveTimesheets to argument - 11Sep25 - Karthik
def createNewIncrementProgressionDetails(employeeData, employmentRecord, workScheduleDayModelAssignments, priorServiceRecord, nesaStatus, empJobHistory, inactiveTimesheets) {
...
//    def wsr = employmentRecord.jobInfoNav.EmpJob.workscheduleCode.text()
	...
	//Added Inactive Timesheets - 11Sep25 - Karthik
    if (inactiveTimesheets.EmployeeTime) {
        employeeLeaves.addAll(inactiveTimesheets.EmployeeTime)
    }
...
//            }
        ...
        //8Sep25 - Partial Leave Case
        //boolean fullDayLeave = false
        //BigDecimal leaveHoursForDay = BigDecimal.ZERO

        //BigDecimal plannedHoursForDay = new BigDecimal(dailyWorkingHoursDouble.toString())
        BigDecimal plannedHoursForDay = BigDecimal.valueOf(dailyWorkingHoursDouble).setScale(1, RoundingMode.HALF_UP) //To Solve the more digits after decimal issue in Partial Leave hours

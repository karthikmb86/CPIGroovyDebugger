            ...
            employeeLeaves.each { node ->
                ...
                if (!tt.startsWith('7')) {
                    ...
                    if (s.equals(e) && s.equals(date)) {
                        ...
                        if (qty && qty.trim()) {
                            try {
                                def roundedQty = new BigDecimal(qty.trim()).setScale(1, RoundingMode.HALF_UP)
                                leaveHoursForDay = leaveHoursForDay.add(roundedQty)
                                //leaveHoursForDay = leaveHoursForDay.add(new BigDecimal(qty.trim())) //19Sep25 - Commenting since a rounding up to one decimal place is required. This was causing more than one digits after decimal place for partial hours.
                            } catch (ignored) {
